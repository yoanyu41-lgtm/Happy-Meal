...
@section('title', __('Create Account') . ' — Happy Meal')
    ...
    <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 20px; padding: 2.5rem 2rem; box-shadow: var(--card-shadow);">
        ...
        <div style="text-align: center; margin-bottom: 2rem;">
            <div style="width: 60px; height: 60px; background: linear-gradient(135deg, var(--primary-color), #8b5cf6); border-radius: 16px; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem;">
                <span class="material-icons-round" style="color: #fff; font-size: 1.75rem;">person_add</span>
            </div>
            <h1 style="font-size: 1.6rem; font-weight: 800; letter-spacing: -0.5px;">{{ __('Create Account') }}</h1>
            <p style="color: var(--text-secondary); font-size: 0.9rem; margin-top: 0.3rem;">{{ __('Join Happy Meal to track your orders') }}</p>
        </div>
        ...
        <div style="text-align: center; margin-top: 1.25rem; font-size: 0.88rem; color: var(--text-secondary);">
            ...
            <a href="{{ route('customer.login') }}" style="color: var(--primary-color); font-weight: 600; text-decoration: none;">
                {{ __('Sign In') }}
            </a>
        </div>
